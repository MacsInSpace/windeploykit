...
| (not `vendor/aria2-tools/*.tar.gz`) | The first 0.6.0 notarisation was rejected for the **unsigned `aria2c` inside the tar.gz** - Apple unpacks archives. Signing it would not help either: that `aria2c` is a Homebrew bottle linked against `/opt/homebrew/opt/{openssl@3,libssh2,c-ares,sqlite,gettext}` dylibs, so it only runs where Homebrew's aria2 is already installed. **No Homebrew fallback** (Craig, 2026-08-28: both apps stay self-sufficient - the `/opt/homebrew/bin` probe was removed the same day). Until a static, signed `aria2c` is built and bundled, the packaged macOS app has no working aria2 source: `Get-AppAria2BinaryPath` resolves `APP_ARIA2` -> runtime install -> plain `Get-Command aria2c` only. Building that static binary is the open packaging task |
